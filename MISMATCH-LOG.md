...
**Статус (2026-07-20): все 🔴 HIGH (1–5) реализованы** — проверено против прод-билда (config webServer, :3010), 10/10 зелёных, `npm run typecheck` = 0. Добавленные testid'ы: `mobile-nav-toggle`/`mobile-nav-panel`/`mobile-nav-link` (мобильное меню), `book-online-link`+`data-variant` (CTA), `booking-summary-master` (сводка брони). MED/LOW (6–17) — открыты.
