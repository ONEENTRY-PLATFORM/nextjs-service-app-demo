...
4. ✅🔴 **Offer → booking deep-link** — покрыто `tests/e2e/offer-booking.spec.ts`. **Пробел исправлен (2026-07-21):** раньше в корзину клался offer-продукт (набор `offer`, id 310–313), которого нет в каталоге услуг брони (`service`) — преселект визарда no-op'ил и открывался entry-экран. Теперь «Book Offer» ведёт на `/booking?services={productId,…}` с id услуг из `offer_services` (`offerServiceProductIds` → `offerBookingHref` → `useServicesPrefill` → `useBookingPreselect`, приоритет: reschedule → query → корзина); корзина в этом флоу не используется, RTK-запрос категории убран (клик больше не «залипает» до его загрузки). Тест проверяет URL с `?services=`, studio-first флоу и услуги в сводке. *Отдельно:* тот же флоу у home `OfferCard` — код общий, отдельным тестом не покрыт.
