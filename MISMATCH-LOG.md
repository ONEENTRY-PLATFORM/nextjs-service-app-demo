...
**Статус (2026-07-21): закрыт весь список 6–17** (🟠 MED + ⚪ LOW; 18–19 остаются ⛔ неавтоматизируемыми). Итог полного прогона: **98 passed / 2 skipped**, два проекта Playwright (desktop + Mobile Chrome), `npx tsc --noEmit` = 0.
...
- 🟠 **`router.push('/')` после логаута не срабатывает.** Токены чистятся, шапка и `/profile` мгновенно переходят в разлогиненное состояние, но URL остаётся `/profile`: сброс сессии ре-рендерит страницу в auth-wall и размонтирует меню, и переход `next-transition-router` не коммитится. Тот же код в `SignOutButton`. Тест фиксирует реальный контракт (очистка `localStorage` + auth-wall), редирект не проверяет — чинить отдельно, если редирект нужен.
- 🟠 **`og:url` / `og:site_name` теряются на всех страницах** — per-page `openGraph` заменяет корневой объект целиком (см. п.14 ниже). SEO-регрессия мелкая, но реальная: чинится добавлением `url`/`siteName` в `openGraph` страниц.
...
14. ✅⚪ **SEO глубже** — покрыто `tests/e2e/seo.spec.ts` (абсолютные URL метаданных никогда не на CMS-хосте, `og:type` есть, шаблон тайтла `%s | <site>` на дочерней странице, `robots.txt`/`sitemap.xml` = 200 и без `oneentry.cloud`). **Находка:** `og:url`/`og:site_name` из корневых метаданных **не доезжают ни до одной страницы** — объект `openGraph` на уровне страницы (`app/page.tsx`, `app/[handle]/page.tsx` задают только `type`/`title`) заменяет корневой целиком. Тест это фиксирует условно (проверяет то, что реально отдаётся); чинить — добавить `url`/`siteName` в per-page `openGraph`.
