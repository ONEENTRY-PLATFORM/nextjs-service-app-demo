...
>
> **Обновление (2026-07-17): ✅ `home_discounts` заполнен.** Создан набор атрибутов `home_discounts` (id **19**, `typeId: 2` = `forBlocks`) с 6 полями: `bg_image`/`bg_image_mobile` (image → `image_id1`/`image_id2`), `title`/`description`/`phone`/`button_text` (string → `string_id3`…`string_id6`); назначен блоку (id 8, `PUT /api/admin/blocks/8`, `attributeSetId: 19`). Залиты **баннеры-заглушки** `lash-lift.png` + `Mobile/lash-lift-m.png` (пользователь заменит своими позже — просил «любую без текста») и тексты: title «10% Off Online», description «Book your appointment online and save», phone `+971 4 784 0098`, button «Book Now». Проверено публичным SDK (6/6 маркеров) и SSR-HTML главной (секция рендерится, картинка с CDN-пути `…/block/8/images/`). Механика — см. врезку в §11 и память `admin-api-block-attribute-set`. ⚠️ загрузка файла в блок: `POST /api/admin/files?type=block&entity=images&id=8&…&template=1` (для блока `type=block`, не `page`/`admin`).
...
**`price` — число** (`float`, напр. `370`), а не строка «450 AED», как числил план: сверено 2026-07-17, код читает `product.price` верхнего уровня ✅; `duration` (integer, минуты), `specialist_grade` (list), `description` (text), `currency` (string «AED», флаг `isCurrency`) — заполнены ✅ (все 77 с ненулевой ценой — см. врезку ниже);
...
> ⚠️ **Четырёхзначные цены обнулялись (найдено и исправлено 2026-07-17).** 6 услуг из 77 стояли с `price = 0` — ровно те, у которых `downtown.premium` ≥ 1000: Airtouch (240) 1100, Global Blonde (241) 1250, Keratin Treatment (248) 1000, Bridal Makeup (262) 1000, HIFU (274) 1750, Bridal Henna (300) 1000. Все услуги дешевле 1000 сохранились верно — похоже на разделитель тысяч при ручном вводе («1 100» → 0). Ноль не косметика: букинг считает «from»-цену мастера как MIN по его услугам (`app/booking/booking-data.ts`), поэтому одна нулевая услуга роняла карточку в «FROM 0». Исправлено скриптом `.claude/temp/fill-service-prices.mjs`, проверено `.claude/temp/inspect-zero-prices.mjs` (77/77 с ценой) и визуально. **При расширении цен по салонам (§11.1) четырёхзначных значений будет много — вводить без пробелов/запятых и после заливки прогонять `inspect-zero-prices.mjs`.**
>
> 🔎 **Верхнее `price` — производное, пишется асинхронно.** У атрибута `price` набора 15 стоит флаг `isPrice: true` (внутренний id `float_id7`); admin API пересчитывает верхнеуровневый `product.price` из него **не мгновенно** — GET сразу после PUT возвращает старое значение (0/`null`). Не считать это ошибкой записи: проверять повторным чтением через паузу или публичным SDK.
...
`home_discounts` ✅ **заполнен 2026-07-17** — набор атрибутов 19 (`forBlocks`), баннеры-заглушки + тексты, проверено SDK/SSR (см. §5.1);
...
8. **Слоты поля `interval` формы `order`** — ✅ **РЕАЛИЗОВАНО 2026-07-17** (слоты из расписания). Раньше фронт показывал статичный `TIMES` (`booking-page/constants.ts`) и не зависел от мастера/салона.
   - *Сделано:* слоты берутся из расписания через новый SDK-API (oneentry ≥ 1.0.156). Источник: `master_schedule` выбранного мастера, а при «Any specialist»/до выбора — `salon_time` салона (оба уже заполнены в CMS). Развёртка — утилита `components/layout/booking-page/daySlots.ts` (`expandAttributeTimeIntervals(schedule, { from, to })` на выбранный UTC-день → `HH:MM`-метки старта; UTC согласован с `useBookingSubmit`, который пишет слот через `Date.UTC`). Прокинуто `booking-data` (`BookingMaster.schedule`/`BookingSalon.schedule`) → `useBookingWizard` (`slots`+`hasSchedule`) → `DateTimeStep`.
   - *Логика фолбэка:* `hasSchedule=false` (нет расписания / демо) → статичный `TIMES`; `hasSchedule=true` и день пуст (мастер не работает) → сообщение «No available times on this day», а не вводящая в заблуждение сетка. Смена даты сбрасывает выбранное время (`onDate`).
   - *Проверено:* Nicolas Costa @ Downtown, 23 июля → слоты **10:00–21:00** из `master_schedule` (статичный `TIMES` начинался бы с 09:00 — доказывает источник); юнит-тест `daySlots.test.ts` (5 кейсов: развёртка/сортировка/дедуп/нет расписания/битая дата); tsc 0, eslint 0, jest 65/65.
   - *Прошедшие слоты* ✅ дизейблятся: на выбранном сегодняшнем дне слот со временем ≤ текущего показывается зачёркнутым и некликабельным (прошедшие дни календарь и так не даёт выбрать). Сравнение **однозонное, без UTC-конверсий** — по решению пользователя (2026-07-17): салон и клиенты в одной локали (Дубай), поэтому метка слота, расписание и часы браузера — одно и то же wall-clock время. Проверено: сегодня 21:43 → все слоты 10:00–21:00 недоступны; завтра — все доступны.
   - *⚠️ Осталось (отдельно):* блокировка занятых слотов (`busyTimes`) — публичный SDK отдаёт заказы только текущего юзера, занятость других клиентов оттуда не видна; нужен серверный эндпоинт доступности. Хук `busyTimes` в `DateTimeStep` оставлен под это.
9. **Выбор оплаты** — ✅ **РАБОТАЕТ (обновл. 2026-07-17, Stripe подключён).** Сделан по решению пользователя, хотя в верстке его нет вовсе (шаги `salon/specialist/service/datetime`, `completeBooking()` сразу показывает «Booked!»). Селектор показывается при 2+ **подключённых** аккаунтах. Теперь Stripe подключён: `testMode: true`, `testSettings.status: "connected"`, onboarding завершён, `pk_test_…` есть — хук `usePaymentAccounts.isConnected` при testMode смотрит `testSettings`, поэтому оба аккаунта (Stripe + Cash) проходят. Проверено: (1) селектор с кнопками **Stripe/Cash** виден на шаге сводки (Playwright, тестовый юзер); (2) сквозная онлайн-оплата — `createOrder(stripe)` → **201**, `createSession` → реальный `https://checkout.stripe.com/c/pay/cs_test_…` (в прошлый раз тут был `400 not_connected`). Тестовый заказ отменён.
   - ⚠️ **Осталось поправить в админке Stripe-аккаунта:** `successUrl`/`cancelUrl` сейчас оба = `https://beauty.oneentry.cloud/` (CMS-хост). После оплаты клиент вернётся на хост CMS, а не на страницы сайта `payment_success` (id 120) / `payment_canceled` (id 121). Наш код URL-ы в `createSession` не передаёт — они берутся из настроек аккаунта. Указать их на реальные страницы сайта.
