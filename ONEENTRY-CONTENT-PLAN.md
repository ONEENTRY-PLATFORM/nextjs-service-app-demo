...
>
> **✅ Статус 2026-07-23:** все **285 маркеров заведены** в `system_content` (набор id 2, блок id 9) и **читаются публично** (залито `.claude/temp/fill-system-content.mjs`, бэкап `backup-system-content-2026-07-23.json`). Реконсиляция сделана: `UserForm` переведён на `edit_text`/`cancel_text`; добавлены `auth_required_text`/`verification`/`cancel_booking_text`/`reschedule_text`.
>
