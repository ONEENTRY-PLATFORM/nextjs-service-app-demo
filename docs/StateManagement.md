...
This document describes the state management approach used in the OneEntry Next.js Beauty Salon project.
...
1. **Cart Slice** - Manages selected services and appointment information
2. **Favorites Slice** - Manages user's favorite services and masters
3. **Order Slice** - Manages appointment booking process data
...
RTK Query is used for server state management through the [RTKApi](app\api\api\RTKApi.ts#L67-L67), providing:
...
- Service and category data fetching
...
Manages the service selection state including:

- Service items with quantities
- Appointment information (date, time, master preference)
- Currency settings
- Total calculation

### Favorites Slice

Manages the user's favorite services and masters:
...
Manages the appointment booking process:

- Form data collection
- Service items for the appointment
...
- Selected services and appointment data
- Favorites list
...
const MyComponent = () => {
  const selectedServices = useAppSelector(selectCartItems);
