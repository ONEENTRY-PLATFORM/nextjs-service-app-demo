...
Authorization is a critical component of the beauty salon system. Without it, many features are inaccessible because they require a **JWT session token**.
...
2. **SDK Handles Tokens**
    ...
    - You only need to pass the `refreshToken` during SDK initialization:
      ...
      ```typescript
      reDefine(refreshToken);
      ```

## AuthContext

All authorization logic, including login and registration, is located in the **AuthContext**. This is a React context that centralizes authentication workflows for beauty salon customers.
...
In the beauty salon context, this includes managing customer appointments, service preferences, and booking history.
