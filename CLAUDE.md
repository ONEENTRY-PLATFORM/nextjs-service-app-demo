...
- Известные сущности (сверено с API 2026-07-09, полный отчёт — `temp/ONEENTRY-CONTENT-PLAN.md`):
  ...
  - **Мастера = админы OneEntry** (`getAdminsInfo` / `useGetAdminsQuery`), фильтр «мастер» = заполнен `master_name`; набор атрибутов `master` (для админов): `master_image` (image), `master_rating` (integer), `master_expirience` (string, опечатка — так в админке!), `master_short_description` (string), `master_description` (text), `master_services` (entity — связь на страницы услуг/продукты, код читает `.id`/`.title`), `master_salon` (entity), `master_schedule` (timeInterval), `master_portfolio` (entity). Роут мастера — `/masters/{admin.id}`. Набор `master` заведён; админов с заполненным `master_name` пока **нет** (0 из 2) — страницы specialists/booking работают на демо-ростере. Схема маркеров — префиксная (по таблице п.1.3 плана): при расхождении маркеров код правится под маркеры плана (напр. `services`→`master_services`).
