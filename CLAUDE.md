...
- Известные сущности (снимок 2026-07-09, ключевые статусы обновлены 2026-07-12; живой источник — `temp/ONEENTRY-CONTENT-PLAN.md`):
  ...
  - **`Forms.getFormByMarker`: `attributes` — массив ИЛИ объект** (`{}`, когда у формы нет полей — сейчас так у `reg`/`order`/`contact_us`). Не вызывать методы массива напрямую — нормализовать через `getFormAttributes` (`components/utils/getFormAttributes.ts`). Инспекция: `.claude/temp/inspect-forms.mjs`.
- API меню возвращает `children: []` у каждого пункта — `flatMenuToNested` (`components/utils/flatMenuToNested.ts`) удаляет пустые; не полагаться на «есть поле children = есть подменю» без проверки длины.
...
- **Размещение утилит — по домену потребителей** (не сваливать в `app/utils`): утилиту с потребителями строго в одном layout-домене класть в `components/layout/<домен>/utils/` (как `booking-page/utils/`, `profile-page/utils/`, `salon-page/utils/`); общую для нескольких доменов и дёргаемую из компонентов — в `components/utils/`; общую чисто app-инфраструктурную (SEO/metadata, `constants`, `route`-хелперы, диагностика) — в `app/utils/`. Файл с единственным потребителем-компонентом жить рядом с ним (`buildOAuthState` в `components/forms/`, `serializeJsonLd` в `components/shared/`, `parseOAuthState` в `app/auth/callback/google/`). Импорты — через алиас `@/`.
...
- Серверные обёртки SDK — `app/api/server/<домен>/<имя>.ts` (возврат `{ isError, error?, <доменный-payload>?, [total] }`: полезная нагрузка лежит под доменным ключом — `page`/`block`/`products`/…, а не под общим `data`; напр. `getPageByUrl` → `{ isError, error?, page? }`), экспорт добавлять в `app/api/index.ts`. Исключения из конверта задокументированы у самих функций: `logOutUser` **никогда не бросает** и деградирует в `finally` (у потребителей try/catch не нужен), `updateUserState`/`clearUserState` возвращают `boolean` (мутации `user.state`). Клиентские данные — RTK Query (`app/api/api/RTKApi.ts`) или хуки `app/api/hooks/`.
...
- **`useTransitionRouter` — только внутри `<TransitionProvider>`.** `Header`, `BottomMenu` и `PopupRoot` смонтированы вне него (`app/layout.tsx`), и там хук отдаёт дефолтный контекст с `navigate: () => {}` — программный `router.push` молча не срабатывает (так ломались редиректы в `LogoutMenuItem` и `VerificationForm`; оба переведены на `useRouter` из `next/navigation`). Клики по `<Link>`/`<a href>` не страдают — провайдер делегирует их на уровне документа.
- **`position: fixed` внутри GSAP-обёртки не полноэкранный:** обёртки оставляют инлайновый `transform` даже в покое, а он становится containing block. Модалки/оверлеи из таких поддеревьев (карточка заказа профиля) монтировать через `components/shared/DialogPortal.tsx`.
...
- `components/data/` — временные захардкоженные данные (отзывы, соцсети, градиенты, «Reach out»-контакты) до переноса в CMS: по файлу на набор, каждый типизирован.
- Приватный `/profile` гейтится **на клиенте** (осознанно, не middleware): токены живут в `localStorage`, cookie-сессии нет — серверный middleware-редиректор без неё не сработает. Серверной утечки нет: заказы/данные пользователя защищены user-токеном на уровне SDK, гость видит только каркас (`AuthError`). Вводить серверный гейт — только вместе с cookie-сессией.
