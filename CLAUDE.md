...
- Известные сущности (снимок 2026-07-09, ключевые статусы обновлены 2026-07-12; живой источник — `temp/ONEENTRY-CONTENT-PLAN.md`):
  ...
  - **Блоки**: `home_hero`, `home_catalog`, `home_gallery`, `home_offers_feed`, `home_discounts`, `home_masters`, `reviews_carousel` — есть и привязаны к `home` (2026-07-12). `system_content` и `opening_time` **НЕТ** (404) — код читает эти маркеры и обязан деградировать на фолбэки.
  ...
  - **Продукт = услуга**: в каталоге ~77 услуг-продуктов (набор `a_sets_tpl_catalog_1_…`/`service`: `title`, `description`, `sku`, `duration`, `specialist_grade`; `price` — **строка** вида «450 AED»). Спец-предложения — **4 продукта с набором `offer`** (id 310–313; поля `offer_price`/`offer_sale`/`offer_type`/`offer_sku`/`offer_services` — внутр. `real_id2`/`real_id3`/…/`entity_id4`; `offer_services` заполнен составом по плану §4). ⚠️ код различает оффер по `attributeSetIdentifier === 'service_set'` — сверить с фактическим набором `offer`, иначе `offers-*` их не увидят.
...
- **Механика сохранения — «автосейв целым объектом»** (кнопки Save в UI нет): GET сущность → мутировать → PUT весь объект обратно.
  ...
  - Файл (image/groupOfImages): multipart `POST /api/admin/files?type=admin|page&entity=image&id={id}&edit=false&compress=true`, поле `file` → `[{filename,downloadLink,size,contentType}]`; в PUT положить `[{...fileObj, params:{isImageCompressed:true}}]`. Сущность должна существовать **до** загрузки (файл кладётся под её id).
